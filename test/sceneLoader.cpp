    ...
    float*& radius)
{
    ...
    for (int i=0; i<numCircles; i++) {
        depths[i] = (float)pz[i] / sizez + 1.0f;
    }
    ...
    for (int i=0; i<numCircles; i++) 
    {
        ...
        // radius[i] = 0.05f;
        int index3 = 3 * i;
        // I don't really understand why we need depth here though
        position[index3] = px[i] / (float)sizex;
        position[index3+1] = py[i] / (float)sizey;
        position[index3+2] = pz[i] / (float)sizez;
        color[index3] = 0.f;
        color[index3+1] = 0.f;
        color[index3+2] = 0.5f;
