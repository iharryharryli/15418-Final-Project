...
#define SCAN_BLOCK_DIM BLOCK_W * BLOCK_W
...
__device__ __inline__ void
myShadePixel(int circleIndex, float2 pixelCenter, float3 p, float4& existingColor) {
...
void
CudaRenderer::loadISF(int particleCount, double* px, double *py, double *pz,
...
void print_info(GlobalConstants & params)
{
	printf("\n\nStart Info Printing\n");
...
CudaRenderer::setup() {
	...
	cudaMalloc(&(params.blockQueueSize), sizeof(int) * numblock);
	cudaMalloc(&(params.blockQueue), sizeof(int) * numblock * numCircles);
	//cudaDevicePixelQueue = params.pixelQueue;
	//cudaDevicePixelQueueSize = params.pixelQueueSize;
...
// Right now this function is identical with setup
// We might change it in the future to do more interesting things with it
void
CudaRenderer::setupISF() {
	...
	printf("---------------------------------------------------------\n");
	printf("[ISF]Initializing CUDA for CudaRenderer\n");
	printf("Found %d CUDA devices\n", deviceCount);
...
__global__ void kernelCircle(int limit)
{
	int index = blockIdx.x * blockDim.x + threadIdx.x;
	...
	int hmax = cuConstRendererParams.numblock_h;
	int numCircles = cuConstRendererParams.numCircles;

	int ymin = floor(imageWidth * (p.y - rad)) / BLOCK_W;
	int ymax = floor(imageWidth * (p.y + rad)) / BLOCK_W + 1;
	if (ymin < 0) ymin = 0;
	if (ymax > hmax) ymax = hmax;
	int xmin = floor(imageWidth * (p.x - rad)) / BLOCK_W;
	int xmax = floor(imageWidth * (p.x + rad)) / BLOCK_W + 1;
	if (xmin < 0) xmin = 0;
	if (xmax > hmax) xmax = hmax;

	arr = &(cuConstRendererParams.blockQueue[(ymin * hmax + xmin) * cuConstRendererParams.numCircles + index]);

	int interval = numCircles * (hmax - xmax + xmin);
...
__global__ void kernelAmazing()
{
	...
	__shared__ uint blockQueue[SCAN_BLOCK_DIM];
	...
	int* arr = &(cuConstRendererParams.blockQueue
			 [cuConstRendererParams.numCircles * blockIdx.x]);


	float4 colorAcc = *imgPtr;
	for (int circlebase=0; circlebase < cuConstRendererParams.numCircles; circlebase += SCAN_BLOCK_DIM)
	{

		int circle_i = threadIdx.x + circlebase;
		...
		if (prefixSumInput[threadIdx.x] == 1)
			blockQueue[prefixSumOutput[threadIdx.x]] = circle_i;
			//arr[count+prefixSumOutput[threadIdx.x]] = circle_i;
		//count +=
		 int queue_len = prefixSumOutput[SCAN_BLOCK_DIM-1] + prefixSumInput[SCAN_BLOCK_DIM-1];
...
void CudaRenderer::amazing()
{
	...
	numblock *= numblock;
	kernelAmazing<<<numblock, SCAN_BLOCK_DIM>>>();
	cudaDeviceSynchronize();
